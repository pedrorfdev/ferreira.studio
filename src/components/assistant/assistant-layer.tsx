...
// Mobile: bottom sheet em vez de painel lateral fixo
// Desktop: painel flutuante canto inferior direito
...
async function askGemini(messages: Message[], systemPrompt: string, lang: string): Promise<string> {
    ...
    for (let i = 0; i < MODELS.length; i++) {
        try {
            const chat = ai.chats.create({
                model: MODELS[i],
                history,
                config: { systemInstruction: systemPrompt + langInstruction, maxOutputTokens: 600, temperature: 0.7 },
            })
...
function staticFallback(project: ProjectData, lang: string) {
    return lang === "pt"
...
// Variante mobile — slide up from bottom
const mobilePanel: Variants = {
    hidden: { y: "100%", opacity: 0 },
    visible: { y: "0%", opacity: 1, transition: { duration: 0.35, ease: [0.4, 0, 0.2, 1] } },
...
export function AssistantLayer({ project }: Props) {
    ...
    useEffect(() => { if (isOpen) setTimeout(() => textareaRef.current?.focus(), 300) }, [isOpen])
    ...
    const handleSend = useCallback(async (text?: string) => {
        ...
        try {
            ...
            const reply = await askGemini(history, project.assistant.context, currentLang)
            addMessage({ role: "assistant", content: reply })
        } catch (err) {
            console.error(err)
            addMessage({ role: "assistant", content: staticFallback(project, currentLang) })
            setError(currentLang === "pt" ? "Problema de conexão." : "Connection issue.")
        } finally {
            setLoading(false)
        }
        ...
        <>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-(--color-border-subtle) shrink-0">
                ...
                <div className="flex items-center gap-2 mt-2 md:mt-0">
                    ...
                    <span className="text-xs font-medium text-(--color-text-primary)">{project.title}</span>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3 min-h-0">
                {messages.length === 0 && (
                    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex flex-col gap-2">
                        <p className="text-xs text-(--color-text-tertiary) mb-1">{t.project.askAbout}</p>
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <div className="bg-(--color-bg-tertiary) rounded-xl rounded-bl-sm px-3.5 py-3">
                            <div className="flex gap-1">
                                {[0, 1, 2].map((i) => (
                                    <span key={i} className="w-1.5 h-1.5 rounded-full bg-(--color-accent)/60 animate-bounce"
                ...
                {error && <p className="text-[10px] text-(--color-text-tertiary) text-center">{error}</p>}
                <div ref={endRef} />
            </div>
