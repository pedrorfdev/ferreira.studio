...
export function CargaLayout({ project }: Props) {
    ...
    return (
        <div className="flex flex-col">
                ...
                <div className="relative z-10 max-w-5xl">
                    <SectionReveal>
                        <div className="flex items-center gap-2 mb-5">
                            <Leaf size={12} className="text-(--color-accent)" />
                    ...
                    {project.links?.github && (
                        <SectionReveal delay={0.14}>
                            <a href={project.links.github} target="_blank" rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm border border-(--color-border) text-(--color-text-secondary) hover:border-(--color-accent) transition-colors">
                        ...
                        <div className="p-6 md:p-8 rounded-2xl bg-(--color-bg-secondary) border border-(--color-border) h-full flex flex-col gap-4">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-lg bg-(--color-gold-muted) flex items-center justify-center shrink-0">
                                    <Sun size={14} className="text-(--color-gold)" />
                                </div>
            ...
            <section className="px-6 md:px-16 mb-16 max-w-6xl mx-auto w-full">
                <SectionReveal>
                    <div className="p-6 md:p-8 rounded-2xl border border-(--color-gold)/20 bg-(--color-gold-muted) flex flex-col md:flex-row gap-5 md:gap-8 items-start">
                        <div className="flex items-center gap-2 shrink-0">
                            <div className="w-9 h-9 rounded-xl bg-(--color-gold) flex items-center justify-center">
                                <Lightbulb size={16} className="text-white" />
                            </div>
                            <span className="text-xs uppercase tracking-[0.16em] text-(--color-gold) md:hidden">
                                {t.project.sections.analysis}
                            </span>
                        </div>
                        <div>
                            <span className="text-xs uppercase tracking-[0.16em] text-(--color-gold) hidden md:block mb-2">
                                {t.project.sections.analysis}
                            </span>
                            <h2 className="font-display text-lg md:text-2xl font-semibold tracking-[-0.02em] text-(--color-text-primary) leading-snug mb-3">
            ...
            <section className="px-6 md:px-16 mb-20 max-w-4xl mx-auto w-full border-t border-(--color-border-subtle) pt-16">
                ...
                <div className="relative">
                    <div className="absolute left-4 top-0 bottom-0 w-px bg-(--color-border) hidden md:block" />
                            ...
                            return (
                                <SectionReveal key={i} delay={i * 0.07}>
                                    <div className="flex items-start gap-5 md:gap-6">
                                        <div className={cn("w-9 h-9 rounded-full border-2 flex items-center justify-center shrink-0 z-10 bg-(--color-bg-primary)",
                                            i === 0 ? "border-(--color-accent) bg-(--color-accent)" : "border-(--color-border)")}>
                        ...
                        <SectionReveal key={i} delay={i * 0.07}>
                            <div className="rounded-xl border border-(--color-border) overflow-hidden">
                                <button onClick={() => setOpen(open === i ? null : i)}
                                    className="w-full flex items-center justify-between p-5 cursor-pointer bg-(--color-bg-secondary) hover:bg-(--color-bg-tertiary) transition-colors">
                                    <span className="font-display text-base font-semibold text-(--color-text-primary) text-left">
                                        {dec.title}
                                    </span>
                                    <motion.div animate={{ rotate: open === i ? 180 : 0 }} transition={{ duration: 0.2 }}>
                                        <ChevronDown size={16} className="text-(--color-text-tertiary) shrink-0" />
                                    </motion.div>
                                </button>
                                <AnimatePresence>
                                    {open === i && (
                                        <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }}
                                            ...
                                            <div className="px-5 pb-5 bg-(--color-bg-secondary)">
            ...
            <section className="px-6 md:px-16 max-w-6xl mx-auto w-full border-t border-(--color-border-subtle) pt-16">
                <SectionReveal>
                    ...
                    <h2 className="font-display text-3xl md:text-4xl font-semibold tracking-[-0.02em] text-(--color-text-primary) mb-6">
