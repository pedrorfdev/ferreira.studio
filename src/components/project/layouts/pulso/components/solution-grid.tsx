// solution-grid.tsx — setas fora dos cards, alinhadas corretamente
...
function Arrow({ dir }: { dir: "right" | "down" }) {
  return (
    ...
    >
      {dir === "right" ? (
        <ArrowRight size={14} className="text-(--color-accent)/50" />
      ) : (
        <ArrowDown size={14} className="text-(--color-accent)/50" />
      )}
    </div>
  );
}
...
export function SolutionGrid({ section, eyebrow }: Props) {
  ...
  const row1 = items.slice(3, 6).reverse(); // snake: exibe 5,4,3
  ...
  return (
    <section className="py-32 px-8 md:px-16 max-w-7xl mx-auto">
      ...
      <div className="hidden md:block">
        ...
        <div className="flex items-stretch gap-0 mb-0">
          {row0.map((item, col) => (
            <div key={item.title} className="flex items-center flex-1 min-w-0">
              ...
              {/* Seta direita entre col 0→1 e 1→2 */}
              {col < 2 && (
        ...
        {/* Seta down — alinhada à direita (após col 2) */}
        <div className="flex justify-end pr-0 py-3">
          <Arrow dir="down" />
        </div>

        {/* Row 1: ← ← (visual: item5, item4, item3) */}
        <div className="flex items-stretch gap-0">
          {row1.map((item, col) => (
            <div key={item.title} className="flex items-center flex-1 min-w-0">
              ...
              {/* Seta esquerda — col 0 e 1 (entre item5→4 e item4→3) */}
              {col < 2 && (
      ...
      {/* Mobile — coluna simples com seta para baixo entre cards */}
