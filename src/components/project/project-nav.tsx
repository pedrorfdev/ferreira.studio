...
// Mobile: pill menor, sem menu button separado, close sempre visível
...
interface Props { project: ProjectData; scrollY: number; onClose: () => void }
        ...
        <div className="fixed top-3 md:top-4 left-0 right-0 z-95 flex justify-center pointer-events-none px-4">
            <motion.div
                ...
                className={cn(
                    ...
                    "max-w-[calc(100vw-32px)]",  // Nunca sai da tela no mobile
                    expanded ? "rounded-2xl" : "rounded-full",
                )}
            >
                {/* Menu button — aparece ao rolar, só desktop */}
                <motion.button
                    onClick={toggleMenu}
                    animate={expanded
                        ? { width: "auto", opacity: 1, paddingLeft: 14, paddingRight: 10 }
                        : { width: 0, opacity: 0, paddingLeft: 0, paddingRight: 0 }
                    }
                    transition={{ duration: 0.32, ease: [0.4, 0, 0.2, 1] }}
                    className="hidden md:flex items-center gap-2 h-9 md:h-10 overflow-hidden
                ...
                {expanded && <span className="w-px h-4 bg-(--color-border) shrink-0 hidden md:block" />}

                {/* Título */}
                <div className={cn("flex items-center transition-all duration-400",
                    expanded ? "px-3 md:px-4 h-9 md:h-10" : "px-4 md:px-5 h-8 md:h-9")}>
                    <span className={cn("font-display font-semibold tracking-[-0.01em] text-(--color-text-primary) whitespace-nowrap",
                        expanded ? "text-xs md:text-sm" : "text-[11px] md:text-xs")}>
                        {project.title}
                    </span>
                </div>
                ...
                {/* Controls */}
                <div className={cn("flex items-center gap-2 md:gap-3 transition-all duration-400",
                    expanded ? "px-3 md:px-4 h-9 md:h-10" : "px-3 md:px-4 h-8 md:h-9")}>
                    {/* Theme */}
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        <button onClick={() => lang !== "en" && toggleLang()} title="English"
                            className={cn("text-sm leading-none cursor-pointer transition-opacity select-none",
                                lang === "en" ? "opacity-100" : "opacity-35 hover:opacity-70")}>🇺🇸</button>
                        <button onClick={() => lang !== "pt" && toggleLang()} title="Português"
                            className={cn("text-sm leading-none cursor-pointer transition-opacity select-none",
                                lang === "pt" ? "opacity-100" : "opacity-35 hover:opacity-70")}>🇧🇷</button>
                    </div>

                    <span className="w-px h-3.5 bg-(--color-border) shrink-0" />

                    {/* Close */}
                    <button onClick={onClose} aria-label={t.project.close}
